...
class AttackEvaluator:
    ...
    def evaluate_clean_accuracy(self) -> Dict:
        ...
        print(f"\n[OK] Clean Accuracy: {clean_accuracy * 100:.2f}%")
    ...
    def generate_comparison_plots(self, save_dir: str = './results/attack_evaluation'):
        ...
        print(f"[OK] Comparison plots saved to: {save_path}")
        plt.close()
    ...
    def generate_visual_grid(
        ...
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"[OK] Visual grid saved to: {save_path}")
        plt.close()
...
def main():
    ...
    print(f"[OK] Test loader ready (batch size: {args.batch_size})\n")
